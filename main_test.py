...
def test_02_create_tables_test_database():
    ...
    cursor.execute(
        "CREATE TABLE teams(team_id SERIAL PRIMARY KEY, naam VARCHAR(255), competitie_id INT, FOREIGN KEY(competitie_id) REFERENCES competities(competitie_id));")
    cursor.execute(
        "CREATE TABLE spelers(speler_id SERIAL PRIMARY KEY, naam VARCHAR(255), land VARCHAR(255), leeftijd SMALLINT, statistiek SMALLINT, team_id INT, FOREIGN KEY(team_id) REFERENCES teams(team_id));")
    cursor.close()
    db_connection.commit()
...
def test_20_create_spelers():
    response = client.post(
        "/spelers?spelernaam=Hans&spelerland=Spanje&spelerleeftijd=60&spelerstatistiek=60&teamid=1")
    assert response.status_code == 400


def test_waarde_berekenen_jonge_speler():
    speler = ("1", "John Doe", "Nederland", 18, 80, 1)
    assert waarde_berekenen(speler) == 26400000
...
def test_waarde_berekenen_gemiddelde_speler():
    speler = ("1", "John Doe", "Nederland", 22, 80, 1)
    assert waarde_berekenen(speler) == 40000000


def test_21_delete_speler():
    response = client.delete("/spelers/10")
    assert response.status_code == 404
