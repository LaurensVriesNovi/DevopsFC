...
def leeftijf_controle(spelerleeftijd):
    if spelerleeftijd < 16:
        return HTTPException(status_code=400, detail="Speler is te jong (jonger dan 16 jaar)")
    elif spelerleeftijd > 45:
        return HTTPException(status_code=400, detail="Speler is te oud (ouder dan 45 jaar)")

def statestiek_controle(spelerstatistiek):
    if spelerstatistiek < 10:
        return HTTPException(status_code=400, detail="Speler is te slecht (statestiek kleiner dan 10)")
    elif spelerstatistiek > 99:
        return HTTPException(status_code=400, detail="Speler is te goed (statestiek groter dan 99)")
...
def team_id_controle(teamid):
    ...
    if team is None:
        cursor.close()
        return HTTPException(status_code=404, detail="Team niet gevonden")
    cursor.close()
...
def speler_id_controle(speler_id):
    ...
    if speler is None:  # Controleer of de speler niet bestaat
        cursor.close()
        return HTTPException(status_code=404, detail="Speler niet gevonden")
    cursor.close()
...
def competitie_id_controle(competitie_id):
    ...
    if competitie is None:
        cursor.close()
        return HTTPException(status_code=404, detail="Competitie niet gevonden")
    cursor.close()